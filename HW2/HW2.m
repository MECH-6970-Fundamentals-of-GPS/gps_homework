...
ab = zeros(2,1);
[ab, idx1] = LS(y, Hab, ab);
Pab = sigma_y*inv(Hab'*Hab);

Habc = [ones(N,1), x', (x.^2)'];
abc = zeros(3,1);
[abc, idx2] = LS(y, Habc, abc);
Pabc = sigma_y*inv(Habc'*Habc);

Habcd = [ones(N,1), x', (x.^2)', (x.^3)'];
abcd = zeros(4,1);
[abcd, idx3] = LS(y, Habcd, abcd);
Pabcd = sigma_y*inv(Habcd'*Habcd);
...
fprintf('2a) Linear a estimation error: %0.6g\n', Pab(1,1));
fprintf('2b) Quadratic a estimation error: %0.6g\n', Pabc(1,1));
fprintf('2c) Cubic a estimation error: %0.6g\n\n', Pabcd(1,1));
